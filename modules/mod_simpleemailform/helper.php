...
/*
		...
		2012-04-25: DB
		* Fixed problem where files do not upload unless an extension is specified (thanks to Rob Smolenaars!)
*/
...
class modSimpleEmailForm {
	...
	public function uploadAttachment($dir, $uploadAllowed, $errorTxtColor, $successTxtColor, &$message)
	{
		...
		$allowed = FALSE;
		// Capture filename
		$fn = (isset($_FILES['mod_simpleemailform_upload']['name'])) ? basename(strip_tags($_FILES['mod_simpleemailform_upload']['name'])) : '';
		...
		if ($fn) {
			...
			if ($allowed) {
				...
				if ( $_FILES['mod_simpleemailform_upload']['error'] == UPLOAD_ERR_OK ) {
					// Check to make sure file uploaded by upload process
					if ( is_uploaded_file ($_FILES['mod_simpleemailform_upload']['tmp_name'] ) ) {
						// Set filename to current directory
						$copyfile = $dir . DIRECTORY_SEPARATOR . $fn;
						// Copy file
						if ( move_uploaded_file ($_FILES['mod_simpleemailform_upload']['tmp_name'], $copyfile) ) {
							// Save name of file
	...
	public function formatRow ($name, $size, $label, $error = NULL, $maxx = 255) 
	{
		...
		$row .= "<input type='text' name='$name' id='$name' size='$size' value='$value' maxlength='$maxx'" . $this->_inputClass . "/>";
	...
	public function sendResults(_SimpleEmailForm &$msg, $field, $txtLabel, $fromlabel)
	{
		...
		ob_start();
		// Build Body
		$msg->subject = (isset($_POST['mod_simpleemailform_subject'])) ? htmlspecialchars($_POST['mod_simpleemailform_subject']) : $msg->subject;
		$msg->body =  $fromlabel . ': ' . htmlspecialchars($msg->from);
		$msg->body .= (isset($_POST['mod_simpleemailform_field1']))    ? "\n" . $field[1]['label'] . ': ' . htmlspecialchars($_POST['mod_simpleemailform_field1']) : "";
		$msg->body .= (isset($_POST['mod_simpleemailform_field2']))    ? "\n" . $field[2]['label'] . ': ' . htmlspecialchars($_POST['mod_simpleemailform_field2']) : "";
		$msg->body .= (isset($_POST['mod_simpleemailform_field3']))    ? "\n" . $field[3]['label'] . ': ' . htmlspecialchars($_POST['mod_simpleemailform_field3']) : "";
		$msg->body .= (isset($_POST['mod_simpleemailform_field4']))    ? "\n" . $field[4]['label'] . ': ' . htmlspecialchars($_POST['mod_simpleemailform_field4']) : "";
		$msg->body .= (isset($_POST['mod_simpleemailform_field5']))    ? "\n" . $field[5]['label'] . ': ' . htmlspecialchars($_POST['mod_simpleemailform_field5']) : "";
		$msg->body .= (isset($_POST['mod_simpleemailform_textarea']))  ? "\n" . $txtLabel          . ":\n" . htmlspecialchars($_POST['mod_simpleemailform_textarea']) : "";
		// Strip slashes
		...
		try {
			$sent = $message->send();
			$msg->copyMe = (isset($_POST['mod_simpleemailform_copyMe'])) ? (int) $_POST['mod_simpleemailform_copyMe'] : 0;
	...
	{
		$message = '';
		$currentSession = JSession::getInstance('mod_SimpleEmailForm', array());
		...
		if (isset($_POST['mod_simpleemailform_submit'])) {

			$this->_msg->from = strip_tags($_POST['mod_simpleemailform_from_field']);
			...
			if (!$this->_emailCheck || $this->isEmailAddress($this->_msg->from)) {
				...
				$requiredCheck = TRUE;
				for ($x = 1; $x < $this->_maxFields; $x++) {
					$fieldLabel = 'mod_simpleemailform_field' . $x;
				...
				if ($this->_txtActive == 'R') {
						if (!isset($_POST['mod_simpleemailform_textarea']) || $_POST['mod_simpleemailform_textarea'] == NULL) {
							$requiredCheck = FALSE;
				...
				if ($requiredCheck) {
					...
					if ($this->_useCaptcha != 'N') {
						$captchaPhrase = $currentSession->get('captchaPhrase');
						if (isset($_POST['mod_simpleemailform_captcha']) && $_POST['mod_simpleemailform_captcha'] == $captchaPhrase) {
				...
				$this->_badEmail .= $this->formatErrorMessage($this->_errorTxtColor, $this->_transLang['MOD_SIMPLEEMAILFORM_email_invalid']);
			}
		} elseif (isset($_POST['mod_simpleemailform_reset'])) {
			$this->autoResetForm();
		}
		...
		if ($this->_testMode == 'Y') {
			$this->_output .= '<pre>';
			$this->_output .= var_export($this->_msg);
			$this->_output .= '</pre>';
		}
		$this->_output .= "<form method='post' "
						. "action='" . $this->_anchor . "' "
						. "name='_SimpleEmailForm' "
						. "id='_SimpleEmailForm' "
						. "enctype='multipart/form-data'>\n";
		$this->_output .= "<table" . $this->_tableClass . ">\n";
		$this->_output .= $this->formatRow('mod_simpleemailform_from_field', $this->_fromsize, $this->_fromlabel, $this->_badEmail);
		if (!isset($_POST['mod_simpleemailform_subject'])) { $_POST['mod_simpleemailform_subject'] = $this->_subjectline; }
		$this->_output .= $this->formatRow('mod_simpleemailform_subject', $this->_subjectsize, $this->_subjectlabel);
		...
		if ($this->_txtActive != 'N') {
			$value = (isset($_POST['mod_simpleemailform_textarea'])) ? htmlspecialchars($_POST['mod_simpleemailform_textarea']) : "";
			...
			$this->_output .= "<textarea "
							. $this->_inputClass
							. "name='mod_simpleemailform_textarea' "
							. "id='textarea' "
							. "rows='" . $this->_txtRows . "' "
							. "cols='" . $this->_txtCols . "'>"
							. stripslashes($value)
		...
		if ($this->_uploadActive == 'Y') {
			...
			$this->_output .= "<input "
							. $this->_inputClass
							. "type=file "
							. "name='mod_simpleemailform_upload' "
							. "enctype='multipart/form-data' />";
		...
		if ($this->_useCaptcha != 'N') {
			try {
				...
				$currentSession->set('captchaPhrase', $phrase);
				...
				$this->_output .= "<input name='mod_simpleemailform_captcha' "
								. $this->_inputClass
								. "id='mod_simpleemailform_captcha' "
								. "type='text' "
		...
		if ($this->_copymeActive == 'Y') {
			...
			$this->_output .= "<input "
							. $this->_inputClass . " "
							. "type='checkbox' "
							. "name='mod_simpleemailform_copyMe' "
							. "id='copyMe' "
							. "value='1' />" 
							. $this->_copymeLabel 
							. "</td>";
		...
		$this->_output .= "<input " 
						. $this->_inputClass . " "
						. "type='submit' "
						. "name='mod_simpleemailform_submit' "
						. "id='submit' "
		...
		$this->_output .= "<input " 
						. $this->_inputClass . " "
						. "type='submit' "
						. "name='mod_simpleemailform_reset' "
						. "id='reset' "
